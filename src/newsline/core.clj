...
(pos-filter nvaa #"(NN|VB|JJ|RB)") ;Filter nouns, verbs, adjectives and adverbs
  ...
  (if (not (checkWord wordnetWordID)) 
    (do
      (if (empty? prevWord) (def weight 1) 
        (def weight (math/abs (* (/ 1000.0 (- (.getOffset wordnetWordID) (.getOffset prevWordID))) activation))))
      (def wordHash (hash-map :wordNet w :word (:lemma w) :id wordnetWordID :value weight))
...
(defn synsets
  ...
  (addSynsets newsynsets word value)
)
...
(defn printBasketlemma
  "Prints the lemma from the wordnet Word in the WordBasket."
  [basket]
  (def a basket)
  (def basketCount (count basket))
  (loop [i 0]
    (when (< i basketCount)
      (print (:gloss ((first a) :wordNet))"-")
      (println (.getOffset ((first a) :id)))
      (def a (rest a))
      (recur (inc i))
      ))
...
(defn -main
  ...
  (def sentences (get-sentences s))
	(def tokens (nvaa (pos-tag (tokenize s))))
      ...
      (loop  [j 0]
        (when (< j senseNumbers)
          (if (not (empty? wordnetWord)) 
            (do
              (addWord (first wordnetWord) 1)
          ;(if (not (= wordnetWord nil)) (println (:lemma wordnetWord)))
              (synsets (first wordnetWord) ((first wordBasket) :value))
  ...
  (loop [i 0]
    (when (< i basketCount)
      (def wordnetWord ((first basketCopy) :wordNet))
      (synsets wordnetWord ((first basketCopy) :value))
      (def basketCopy (rest basketCopy))
      (recur (inc i))
      ))
  (filterWords 2.0)
  ...
  ;   (when (< i basketCount)
  ;     (def wordnetWord ((first basketCopy) :wordNet))
  ;     (synsets wordnetWord ((first wordBasket) :value))
  ;     (def basketCopy (rest basketCopy))
  ;     (recur (inc i))
  ;     ))
  ; (filterWords 5.0)
